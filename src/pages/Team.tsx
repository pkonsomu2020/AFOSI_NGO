...
import { ChevronDown, Users, Award, Briefcase, Monitor, BarChart2, DollarSign, Layers } from "lucide-react";
...
// Card with name below — portrait style
const MemberCard = ({ member }: { member: TeamMember }) => {
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <div className="relative rounded-2xl overflow-hidden aspect-[3/4] mb-3 shadow-sm bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/10">
        {!imgError ? (
          <img
            src={member.image}
            alt={member.name}
          ...
          <div className="w-full h-full flex items-center justify-center">
            <span className="text-5xl font-black text-orange-400/50">{initials}</span>
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-orange-600/70 via-orange-500/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400" />
      </div>
...
// Static grid for board/management
...
const DeptAccordion = ({ dept, defaultOpen = false }: { dept: typeof departments[0]; defaultOpen?: boolean }) => {
  ...
  return (
    <div className="border border-border rounded-2xl overflow-hidden mb-3">
      <button
        ...
        className="w-full flex items-center justify-between px-6 py-4 bg-card hover:bg-accent/40 transition-colors"
      >
          ...
          >
            <div className="px-6 py-6 bg-background">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-5">
                {dept.members.map(m => <MemberCard key={m.name} member={m} />)}
              </div>
            </div>
          </motion.div>
        )}
      ...
      <div className="container mx-auto px-4 max-w-7xl pt-32 pb-20">
        ...
        >
          ...
          <p className="text-muted-foreground mt-4 max-w-2xl text-base">
            A passionate group of professionals, volunteers, and change-makers dedicated to creating sustainable impact across Kenya.
          </p>
        </motion.div>
        ...
        <div className="mb-14">
          ...
          <MemberGrid members={management} />
        </div>

        {/* Core Team — Department Accordions */}
        <div className="mb-14">
          <SectionHeading icon={Users} label="Core Team" />
          {departments.map((dept, i) => (
            <DeptAccordion key={dept.name} dept={dept} defaultOpen={i === 0} />
          ))}
        </div>
      </div>
    </div>
